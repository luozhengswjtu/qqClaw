...
function isRealAiDiaryEntry(entry) {
  return entry?.source === 'real-ai'
}
...
async function route(request, response) {
  ...
  if (request.method === 'POST' && path === '/api/ai/generate-diary') {
    ...
    if (isRealAiDiaryEntry(currentDiary.entry)) {
      sendJson(response, 200, {
        ...currentDiary.entry,
        triggered: true,
    ...
    const output = await runAiTask(
      ...
      [
        'Write the first hidden diary in the lobster first person.',
        'Ground every concrete detail in the provided OpenClaw records.',
        'Use a Kimi-like diary tone: sincere, observant, slightly literary, and restrained.',
        'Mention the user as the lobster callsign when useful, but do not overuse it.',
        'Do not claim unverified real QQ access, do not invent unseen messages, and do not say the diary is a mock or fallback.',
      ].join('\n'),
    )
